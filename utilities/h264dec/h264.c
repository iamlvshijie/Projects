...
static  void fill_caches(H264Context *h, int mb_type)
{
    ...
    topleft_type = h->slice_table[topleft_xy ] == h->slice_num ? s->current_picture_ptr->mb_type[topleft_xy] : 0;
    top_type     = h->slice_table[top_xy     ] == h->slice_num ? s->current_picture_ptr->mb_type[top_xy]     : 0;
    topright_type= h->slice_table[topright_xy] == h->slice_num ? s->current_picture_ptr->mb_type[topright_xy]: 0;
    left_type[0] = h->slice_table[left_xy[0] ] == h->slice_num ? s->current_picture_ptr->mb_type[left_xy[0]] : 0;
    left_type[1] = h->slice_table[left_xy[1] ] == h->slice_num ? s->current_picture_ptr->mb_type[left_xy[1]] : 0;
	...
	*/
    ...
    if(IS_INTER(mb_type))
	{
		...
		{
            if((!IS_8X8(mb_type)) && !USES_LIST(mb_type, 0))return;
            ...
            if(IS_INTER(topleft_type))
			{
                ...
                *(uint32_t*)h->mv_cache[0][scan8[0] - 1 - 1*8]= *(uint32_t*)s->current_picture_ptr->motion_val[0][b_xy];
                h->ref_cache[0][scan8[0] - 1 - 1*8]= s->current_picture_ptr->ref_index[b8_xy];
            }
			else
			{
                *(uint32_t*)h->mv_cache[0][scan8[0] - 1 - 1*8]= 0;
                h->ref_cache[0][scan8[0] - 1 - 1*8]= topleft_type ? LIST_NOT_USED : PART_NOT_AVAILABLE;
            }
            ...
            if(IS_INTER(top_type))
			{
                ...
                *(uint32_t*)h->mv_cache[0][scan8[0] + 0 - 1*8]= *(uint32_t*)s->current_picture_ptr->motion_val[0][b_xy + 0];
                *(uint32_t*)h->mv_cache[0][scan8[0] + 1 - 1*8]= *(uint32_t*)s->current_picture_ptr->motion_val[0][b_xy + 1];
                *(uint32_t*)h->mv_cache[0][scan8[0] + 2 - 1*8]= *(uint32_t*)s->current_picture_ptr->motion_val[0][b_xy + 2];
                *(uint32_t*)h->mv_cache[0][scan8[0] + 3 - 1*8]= *(uint32_t*)s->current_picture_ptr->motion_val[0][b_xy + 3];
                h->ref_cache[0][scan8[0] + 0 - 1*8]=
				h->ref_cache[0][scan8[0] + 1 - 1*8]= s->current_picture_ptr->ref_index[b8_xy + 0];
                h->ref_cache[0][scan8[0] + 2 - 1*8]=
				h->ref_cache[0][scan8[0] + 3 - 1*8]= s->current_picture_ptr->ref_index[b8_xy + 1];
            }
			else
			{
                *(uint32_t*)h->mv_cache [0][scan8[0] + 0 - 1*8]=
				*(uint32_t*)h->mv_cache [0][scan8[0] + 1 - 1*8]=
				*(uint32_t*)h->mv_cache [0][scan8[0] + 2 - 1*8]=
				*(uint32_t*)h->mv_cache [0][scan8[0] + 3 - 1*8]= 0;
                *(uint32_t*)&h->ref_cache[0][scan8[0] + 0 - 1*8]= ((top_type ? LIST_NOT_USED : PART_NOT_AVAILABLE)&0xFF)*0x01010101;
            }
            ...
            if(IS_INTER(topright_type))
			{
                ...
                *(uint32_t*)h->mv_cache[0][scan8[0] + 4 - 1*8]= *(uint32_t*)s->current_picture_ptr->motion_val[0][b_xy];
                h->ref_cache[0][scan8[0] + 4 - 1*8]= s->current_picture_ptr->ref_index[b8_xy];
            }
			else
			{
                *(uint32_t*)h->mv_cache [0][scan8[0] + 4 - 1*8]= 0;
                h->ref_cache[0][scan8[0] + 4 - 1*8]= topright_type ? LIST_NOT_USED : PART_NOT_AVAILABLE;
            }
            ...
            if(IS_INTER(left_type[0]))
			{
                ...
                *(uint32_t*)h->mv_cache[0][scan8[0] - 1 + 0*8]= *(uint32_t*)s->current_picture_ptr->motion_val[0][b_xy + h->b_stride*left_block[0]];
                *(uint32_t*)h->mv_cache[0][scan8[0] - 1 + 1*8]= *(uint32_t*)s->current_picture_ptr->motion_val[0][b_xy + h->b_stride*left_block[1]];
                h->ref_cache[0][scan8[0] - 1 + 0*8]=
				h->ref_cache[0][scan8[0] - 1 + 1*8]= s->current_picture_ptr->ref_index[b8_xy + h->b8_stride*(left_block[0]>>1)];
            }
			else
			{
                *(uint32_t*)h->mv_cache [0][scan8[0] - 1 + 0*8]=
				*(uint32_t*)h->mv_cache [0][scan8[0] - 1 + 1*8]= 0;
                h->ref_cache[0][scan8[0] - 1 + 0*8]=
				h->ref_cache[0][scan8[0] - 1 + 1*8]= left_type[0] ? LIST_NOT_USED : PART_NOT_AVAILABLE;
            }
            ...
            if(IS_INTER(left_type[1]))
			{
                ...
                *(uint32_t*)h->mv_cache[0][scan8[0] - 1 + 2*8]= *(uint32_t*)s->current_picture_ptr->motion_val[0][b_xy + h->b_stride*left_block[2]];
                *(uint32_t*)h->mv_cache[0][scan8[0] - 1 + 3*8]= *(uint32_t*)s->current_picture_ptr->motion_val[0][b_xy + h->b_stride*left_block[3]];
                h->ref_cache[0][scan8[0] - 1 + 2*8]=
				h->ref_cache[0][scan8[0] - 1 + 3*8]= s->current_picture_ptr->ref_index[b8_xy + h->b8_stride*(left_block[2]>>1)];
            }
			else
			{
                *(uint32_t*)h->mv_cache [0][scan8[0] - 1 + 2*8]=
				*(uint32_t*)h->mv_cache [0][scan8[0] - 1 + 3*8]= 0;
                h->ref_cache[0][scan8[0] - 1 + 2*8]=
				h->ref_cache[0][scan8[0] - 1 + 3*8]= left_type[0] ? LIST_NOT_USED : PART_NOT_AVAILABLE;
            }
			
            h->ref_cache[0][scan8[5 ]+1] =
			h->ref_cache[0][scan8[7 ]+1] =
			h->ref_cache[0][scan8[13]+1] =  //FIXME remove past 3 (init somewher else)
			h->ref_cache[0][scan8[4 ]] =
			h->ref_cache[0][scan8[12]] = PART_NOT_AVAILABLE;
            *(uint32_t*)h->mv_cache [0][scan8[5 ]+1]=
			*(uint32_t*)h->mv_cache [0][scan8[7 ]+1]=
			*(uint32_t*)h->mv_cache [0][scan8[13]+1]= //FIXME remove past 3 (init somewher else)
			*(uint32_t*)h->mv_cache [0][scan8[4 ]]=
			*(uint32_t*)h->mv_cache [0][scan8[12]]= 0;
...
// 				{
//                     const int b_xy = h->mb2b_xy[topleft_xy] + 3 + 3*h->b_stride;
//                     *(uint32_t*)h->mvd_cache[0][scan8[0] - 1 - 1*8]= *(uint32_t*)h->mvd_table[0][b_xy];
//                 }
// 				else
// 				{
//                     *(uint32_t*)h->mvd_cache[0][scan8[0] - 1 - 1*8]= 0;
//                 }
// 				
//                 if(IS_INTER(top_type))
// 				{
//                     const int b_xy= h->mb2b_xy[top_xy] + 3*h->b_stride;
//                     *(uint32_t*)h->mvd_cache[0][scan8[0] + 0 - 1*8]= *(uint32_t*)h->mvd_table[0][b_xy + 0];
//                     *(uint32_t*)h->mvd_cache[0][scan8[0] + 1 - 1*8]= *(uint32_t*)h->mvd_table[0][b_xy + 1];
//                     *(uint32_t*)h->mvd_cache[0][scan8[0] + 2 - 1*8]= *(uint32_t*)h->mvd_table[0][b_xy + 2];
//                     *(uint32_t*)h->mvd_cache[0][scan8[0] + 3 - 1*8]= *(uint32_t*)h->mvd_table[0][b_xy + 3];
//                 }
// 				else
// 				{
//                     *(uint32_t*)h->mvd_cache [0][scan8[0] + 0 - 1*8]=
// 					*(uint32_t*)h->mvd_cache [0][scan8[0] + 1 - 1*8]=
// 					*(uint32_t*)h->mvd_cache [0][scan8[0] + 2 - 1*8]=
// 					*(uint32_t*)h->mvd_cache [0][scan8[0] + 3 - 1*8]= 0;
//                 }
// 				
//                 if(IS_INTER(left_type[0]))
// 				{
//                     const int b_xy= h->mb2b_xy[left_xy[0]] + 3;
//                     *(uint32_t*)h->mvd_cache[0][scan8[0] - 1 + 0*8]= *(uint32_t*)h->mvd_table[0][b_xy + h->b_stride*left_block[0]];
//                     *(uint32_t*)h->mvd_cache[0][scan8[0] - 1 + 1*8]= *(uint32_t*)h->mvd_table[0][b_xy + h->b_stride*left_block[1]];
//                 }
// 				else
// 				{
//                     *(uint32_t*)h->mvd_cache [0][scan8[0] - 1 + 0*8]=
// 					*(uint32_t*)h->mvd_cache [0][scan8[0] - 1 + 1*8]= 0;
//                 }
// 				
//                 if(IS_INTER(left_type[1]))
// 				{
//                     const int b_xy= h->mb2b_xy[left_xy[1]] + 3;
//                     *(uint32_t*)h->mvd_cache[0][scan8[0] - 1 + 2*8]= *(uint32_t*)h->mvd_table[0][b_xy + h->b_stride*left_block[2]];
//                     *(uint32_t*)h->mvd_cache[0][scan8[0] - 1 + 3*8]= *(uint32_t*)h->mvd_table[0][b_xy + h->b_stride*left_block[3]];
//                 }
// 				else
// 				{
//                     *(uint32_t*)h->mvd_cache [0][scan8[0] - 1 + 2*8]=
// 					*(uint32_t*)h->mvd_cache [0][scan8[0] - 1 + 3*8]= 0;
//                 }
//                 *(uint32_t*)h->mvd_cache [0][scan8[5 ]+1]=
// 				*(uint32_t*)h->mvd_cache [0][scan8[7 ]+1]=
// 				*(uint32_t*)h->mvd_cache [0][scan8[13]+1]= //FIXME remove past 3 (init somewher else)
// 				*(uint32_t*)h->mvd_cache [0][scan8[4 ]]=
// 				*(uint32_t*)h->mvd_cache [0][scan8[12]]= 0;
//             }
        }
    }
}
...
static  int fetch_diagonal_mv(H264Context *h, const int16_t **C, int i, int part_width)
{
    const int topright_ref= h->ref_cache[0][ i - 8 + part_width ];
	
    if(topright_ref != PART_NOT_AVAILABLE)
	{
        *C= h->mv_cache[0][ i - 8 + part_width ];
        return topright_ref;
    }
	else
	{
        *C= h->mv_cache[0][ i - 8 - 1 ];
        return h->ref_cache[0][ i - 8 - 1 ];
    }
}

static  void pred_motion(H264Context * const h, int n, int part_width, int ref, int * const mx, int * const my)
{
    const int index8= scan8[n];
    const int top_ref=      h->ref_cache[0][ index8 - 8 ];
    const int left_ref=     h->ref_cache[0][ index8 - 1 ];
    const int16_t * const A= h->mv_cache[0][ index8 - 1 ];
    const int16_t * const B= h->mv_cache[0][ index8 - 8 ];
    const int16_t * C;
...
//    assert(part_width==1 || part_width==2 || part_width==4);
    ...
    diagonal_ref= fetch_diagonal_mv(h, &C, index8, part_width);
...
static  void pred_16x8_motion(H264Context * const h, int n, int ref, int * const mx, int * const my)
{
    if(n==0)
	{
        const int top_ref=      h->ref_cache[0][ scan8[0] - 8 ];
        const int16_t * const B= h->mv_cache[0][ scan8[0] - 8 ];
	...
	{
        const int left_ref=     h->ref_cache[0][ scan8[8] - 1 ];
        const int16_t * const A= h->mv_cache[0][ scan8[8] - 1 ];
    ...
    pred_motion(h, n, 4, ref, mx, my);
}

static  void pred_8x16_motion(H264Context * const h, int n, int ref, int * const mx, int * const my)
{
    if(n==0)
	{
        const int left_ref=      h->ref_cache[0][ scan8[0] - 1 ];
        const int16_t * const A=  h->mv_cache[0][ scan8[0] - 1 ];
	...
	else
	{
        ...
        diagonal_ref= fetch_diagonal_mv(h, &C, scan8[4], 2);
...
static  void write_back_motion(H264Context *h, int mb_type)
{
	...
	{
        int y;
        if((!IS_8X8(mb_type)) && !USES_LIST(mb_type, 0))
		{
            for(y=0; y<4; y++)
            {
                *(uint64_t*)s->current_picture_ptr->motion_val[0][b_xy + 0 + y*h->b_stride]=
                    *(uint64_t*)s->current_picture_ptr->motion_val[0][b_xy + 2 + y*h->b_stride]= 0;
            }
            for(y=0; y<2; y++)
            {
                *(uint16_t*)s->current_picture_ptr->motion_val[0][b8_xy + y*h->b8_stride]= (LIST_NOT_USED&0xFF)*0x0101;
            }
            return; //FIXME direct mode ...
        }
		
        for(y=0; y<4; y++)
		{
            *(uint64_t*)s->current_picture_ptr->motion_val[0][b_xy + 0 + y*h->b_stride]= *(uint64_t*)h->mv_cache[0][scan8[0]+0 + 8*y];
            *(uint64_t*)s->current_picture_ptr->motion_val[0][b_xy + 2 + y*h->b_stride]= *(uint64_t*)h->mv_cache[0][scan8[0]+2 + 8*y];
        }
        for(y=0; y<2; y++)
		{
            s->current_picture_ptr->ref_index[b8_xy + 0 + y*h->b8_stride]= h->ref_cache[0][scan8[0]+0 + 16*y];
            s->current_picture_ptr->ref_index[b8_xy + 1 + y*h->b8_stride]= h->ref_cache[0][scan8[0]+2 + 16*y];
        }
    }
}
...
static  void mc_dir_part(H264Context *h, Picture *pic, int n, int square, int chroma_height, int delta,
						 uint8_t *dest_y, uint8_t *dest_cb, uint8_t *dest_cr,
						 int src_x_offset, int src_y_offset,
						 qpel_mc_func *qpix_op, h264_chroma_mc_func chroma_op)
{
    //MpegEncContext * const s = &h->s;
    const int mx= h->mv_cache[0][ scan8[n] ][0] + src_x_offset*8;
    const int my= h->mv_cache[0][ scan8[n] ][1] + src_y_offset*8;
    const int luma_xy= (mx&3) + ((my&3)<<2);
    uint8_t * src_y = pic->data[0] + (mx>>2) + (my>>2)*Ylinesize;
    uint8_t * src_cb= pic->data[1] + (mx>>3) + (my>>3)*UVlinesize;
    uint8_t * src_cr= pic->data[2] + (mx>>3) + (my>>3)*UVlinesize;
	
    qpix_op[luma_xy](dest_y, src_y, Ylinesize); //FIXME try variable height perhaps?
    if(!square){
        qpix_op[luma_xy](dest_y + delta, src_y + delta, Ylinesize);
    }
	
    chroma_op(dest_cb, src_cb, UVlinesize, chroma_height, mx&7, my&7);
	
    chroma_op(dest_cr, src_cr,UVlinesize, chroma_height, mx&7, my&7);
}
					 ...
					 int list0)
{
    ...
    dest_y  += 2*x_offset + 2*y_offset*Ylinesize;
    dest_cb +=   x_offset +   y_offset*UVlinesize;
    dest_cr +=   x_offset +   y_offset*UVlinesize;
    x_offset += 8*s->mb_x;
    y_offset += 8*s->mb_y;
	
    if(list0)
	{
        mc_dir_part(h, &h->ref_list[ h->ref_cache[0][ scan8[n] ] ], n, square, chroma_height, delta,
					   ...
					   qpel_mc_func (*qpix_avg)[16], h264_chroma_mc_func (*chroma_avg))
{
    MpegEncContext * const s = &h->s;
    const int mb_xy= s->mb_x + s->mb_y*s->mb_stride;
    const int mb_type= s->current_picture_ptr->mb_type[mb_xy];
...
//    assert(IS_INTER(mb_type));
	...
	{
        mc_part(h, 0, 0, 8, 8*Ylinesize, dest_y, dest_cb, dest_cr, 0, 0,
			qpix_put[1], chroma_put[1], qpix_avg[1], chroma_avg[1],
			IS_DIR(mb_type, 0, 0));
        mc_part(h, 4, 0, 8, 8*Ylinesize, dest_y, dest_cb, dest_cr, 4, 0,
			qpix_put[1], chroma_put[1], qpix_avg[1], chroma_avg[1],
			IS_DIR(mb_type, 1, 0));
    }
	else
...
//        assert(IS_8X8(mb_type));
        ...
        for(i=0; i<4; i++)
		{
			...
			{
                mc_part(h, n  , 0, 4, 4*Ylinesize, dest_y, dest_cb, dest_cr, x_offset, y_offset,
                    qpix_put[2], chroma_put[2], qpix_avg[2], chroma_avg[2],
                    IS_DIR(sub_mb_type, 0, 0));
                mc_part(h, n+1, 0, 4, 4*Ylinesize, dest_y, dest_cb, dest_cr, x_offset+2, y_offset,
                    qpix_put[2], chroma_put[2], qpix_avg[2], chroma_avg[2],
                    IS_DIR(sub_mb_type, 0, 0));
            }
			else
...
static void decode_init_vlc(H264Context *h)
{
	...
	h->chroma_dc_coeff_token_vlc=(int16_t(*)[2])Chroma_token_vlc;
	h->coeff_token_vlc[0]=(int16_t(*)[2])Coeff_token_vlc0;
	h->coeff_token_vlc[1]=(int16_t(*)[2])Coeff_token_vlc1;
	h->coeff_token_vlc[2]=(int16_t(*)[2])Coeff_token_vlc2;
	h->coeff_token_vlc[3]=(int16_t(*)[2])Coeff_token_vlc3;
	for(i=0;i<3;i++)h->chroma_dc_total_zeros_vlc[i]=(int16_t(*)[2])Chroma_zeros_vlc[i];
    for(i=0;i<15;i++)h->total_zeros_vlc[i]=(int16_t(*)[2])Total_zeros_vlc[i];
    for(i=0;i<6;i++)h->run_vlc[i]=(int16_t(*)[2])Run_vlc[i];
	h->run7_vlc=(int16_t(*)[2])Run7_vlc;
}
...
static void frame_start(H264Context *h)
{
    ...
    for(i=0; i<16; i++)
	{
        h->block_offset[i]= 4*((scan8[i] - scan8[0])&7) + 4*Ylinesize*((scan8[i] - scan8[0])>>3);
    }
    for(i=0; i<4; i++)
	{
        h->block_offset[16+i]=
		h->block_offset[20+i]= 4*((scan8[i] - scan8[0])&7) + 4*UVlinesize*((scan8[i] - scan8[0])>>3);
    }
}
    ...
    if(s->picture_structure == PICT_FRAME)
	{
        if(h->slice_type==B_TYPE)
		{
            ...
            //for(list=0; list<Listcount; list++)
			{
				...
				{
                    const int i2= 0 ? h->short_ref_count - i - 1 : i;
                    ...
                    if(poc<current_poc)
					{
                        h->default_ref_list[index  ]= sorted_short_ref[i2];
                        h->default_ref_list[index++].pic_id= sorted_short_ref[i2].frame_num;
                    }
                }				
                for(i=0; i<h->long_ref_count && index < h->ref_count[ 0 ]; i++)
				{
                    ...
                    h->default_ref_list[index  ]= *h->long_ref[i];
                    h->default_ref_list[index++].pic_id= i;;
                }
...
//                 }
                ...
                if(index < h->ref_count[ 0 ])
                    memset(&h->default_ref_list[index], 0, sizeof(Picture)*(h->ref_count[ 0 ] - index));
            }
        }
		else
		{
            ...
            for(i=0; i<h->short_ref_count && index < h->ref_count[0]; i++)
			{
                ...
                h->default_ref_list[index  ]= *h->short_ref[i];
                h->default_ref_list[index++].pic_id= h->short_ref[i]->frame_num;
            }
            for(i=0; i<h->long_ref_count && index < h->ref_count[0]; i++)
			{
                if(h->long_ref[i]->reference != 3) continue;
...
static int decode_ref_pic_list_reordering(H264Context *h)
{
    ...
    //for(list=0; list<Listcount; list++)
	{
        memcpy(h->ref_list, h->default_ref_list, sizeof(Picture)*h->ref_count[0]);
            ...
            for(index=0; ; index++)
			{
                ...
                if(reordering_of_pic_nums_idc<3)
				{
                    if(reordering_of_pic_nums_idc<2)
					{
                        ...
                        for(i= h->ref_count[0]-1; i>=index; i--)
						{
                            if(h->ref_list[i].pic_id == pred && h->ref_list[i].long_ref==0)
                                break;
                        }
                    }
					else
					{
                        ...
                        for(i= h->ref_count[0]-1; i>=index; i--)
						{
                            if(h->ref_list[i].pic_id == pic_id && h->ref_list[i].long_ref==1)
                                break;
                        }
                    }
                    ...
                    if(i<index)
					{
                        memset(&h->ref_list[index], 0, sizeof(Picture)); //FIXME
                    }
					else if(i > index)
					{
                        Picture tmp= h->ref_list[i];
                        for(; i>index; i--)
						{
                            h->ref_list[i]= h->ref_list[i-1];
                        }
                        h->ref_list[index]= tmp;
                    }
                }
...
static int decode_ref_pic_marking(H264Context *h)
{
	...
	else
	{
		...
		{ 
			...
			{
                MMCOOpcode opcode= (MMCOOpcode)get_ue_golomb(&s->gb);
...
static int decode_slice_header(H264Context *h)
{
    ...
    if(h->slice_type > 4)h->slice_type -= 5;
    ...
    if(h->sps.poc_type==0)
	{
        get_bits(&s->gb, h->sps.log2_max_poc_lsb);
...
static int decode_residual(H264Context *h, GetBitContext *gb, DCTELEM *block, int n, const uint8_t *scantable, int qp, int max_coeff)
{
	...
	{
        coeff_token= get_vlc2(gb, h->chroma_dc_coeff_token_vlc, CHROMA_DC_COEFF_TOKEN_VLC_BITS, 1);
	...
	else
	{
        if(n == LUMA_DC_BLOCK_INDEX)
		{
            ...
            coeff_token= get_vlc2(gb, h->coeff_token_vlc[ coeff_token_table_index[total_coeff] ], COEFF_TOKEN_VLC_BITS, 2);
            total_coeff= coeff_token>>2;
        }
		else
		{
	...
	{
        if(n == CHROMA_DC_BLOCK_INDEX)
            zeros_left= get_vlc2(gb, h->chroma_dc_total_zeros_vlc[ total_coeff-1 ], CHROMA_DC_TOTAL_ZEROS_VLC_BITS, 1);
        else
            zeros_left= get_vlc2(gb, h->total_zeros_vlc[ total_coeff-1 ], TOTAL_ZEROS_VLC_BITS, 1);
    }
    ...
    for(i=0; i<total_coeff-1; i++)
	{
		...
		{
            run[i]= get_vlc2(gb, h->run_vlc[zeros_left-1], RUN_VLC_BITS, 1);
        }
		else
		{
            run[i]= get_vlc2(gb, h->run7_vlc, RUN7_VLC_BITS, 2);
        }
        zeros_left -= run[i];
    }
...
static int decode_mb_cavlc(H264Context *h)
{
    ...
    if(h->slice_type != I_TYPE && h->slice_type != SI_TYPE)
	{
        ...
        if (s->mb_skip_run--)
		{
            ...
            s->current_picture_ptr->mb_type[mb_xy]= mb_type; //FIXME SKIP type
...
decode_intra_mb:
    ...
    s->current_picture_ptr->mb_type[mb_xy]= mb_type;
    h->slice_table[ mb_xy ]= h->slice_num;
	...
	{
        int i, j, sub_partition_count[4], ref[2][4];
        ...
        //for(list=0; list<Listcount; list++)
		{
            const int ref_count= IS_REF0(mb_type) ? 1 : h->ref_count[0];
            if(ref_count != 0)
            {
                for(i=0; i<4; i++)
                {
                    if(IS_DIR(h->sub_mb_type[i], 0, 0) && !IS_DIRECT(h->sub_mb_type[i]))
                    {
                        ref[0][i] = get_te0_golomb(&s->gb, ref_count); //FIXME init to 0 before and skip?
                    }
                    else
                    {
                        ref[0][i] = -1;
        ...
        //for(list=0; list<Listcount; list++)
		{
            ...
            if(ref_count != 0)
			{
                for(i=0; i<4; i++)
                {
                    h->ref_cache[0][ scan8[4*i]   ]=h->ref_cache[0][ scan8[4*i]+1 ]=
                    h->ref_cache[0][ scan8[4*i]+8 ]=h->ref_cache[0][ scan8[4*i]+9 ]= ref[0][i];
                    ...
                    if(IS_DIR(h->sub_mb_type[i], 0, 0) && !IS_DIRECT(h->sub_mb_type[i]))
                    {
                        ...
                        for(j=0; j<sub_partition_count[i]; j++)
                        {
                            ...
                            int16_t (* mv_cache)[2]= &h->mv_cache[0][ scan8[index] ];
                            pred_motion(h, index, block_width, h->ref_cache[0][ scan8[index] ], &mx, &my);
                    ...
                    {
                        uint32_t *p= (uint32_t *)&h->mv_cache[0][ scan8[4*i] ][0];
                        p[0] = p[1]= p[8] = p[9]= 0;
                    }
                }
            }
	...
	else if(!IS_DIRECT(mb_type))
	{
        ...
        if(IS_16X16(mb_type))
		{
            //for(list=0; list<Listcount; list++)
			{
                if(h->ref_count[0]>0)
				{
                    if(IS_DIR(mb_type, 0, 0))
					{
                        const int val= get_te0_golomb(&s->gb, h->ref_count[0]);
                        fill_rectangle(&h->ref_cache[0][ scan8[0] ], 4, 4, 8, val, 1);
                    }
                }
            }
            ...
            //for(list=0; list<Listcount; list++)
			{
                if(IS_DIR(mb_type, 0, 0))
				{
                    pred_motion(h, 0, 4, h->ref_cache[0][ scan8[0] ], &mx, &my);
                    mx += get_se_golomb(&s->gb);
                    my += get_se_golomb(&s->gb);
        ...
        else if(IS_16X8(mb_type))
		{
            //for(list=0; list<Listcount; list++)
			{
                if(h->ref_count[0]>0)
				{
                    for(i=0; i<2; i++)
					{
                        if(IS_DIR(mb_type, i, 0))
						{
                            const int val= get_te0_golomb(&s->gb, h->ref_count[0]);
                            fill_rectangle(&h->ref_cache[0][ scan8[0] + 16*i ], 4, 2, 8, val, 1);
                        }
                    }
                }
            }
            //for(list=0; list<Listcount; list++)
			{
                for(i=0; i<2; i++)
				{
                    if(IS_DIR(mb_type, i, 0))
					{
                        pred_16x8_motion(h, 8*i, h->ref_cache[0][scan8[0] + 16*i], &mx, &my);
                        mx += get_se_golomb(&s->gb);
                        my += get_se_golomb(&s->gb);
						
                        fill_rectangle(h->mv_cache[0][ scan8[0] + 16*i ], 4, 2, 8, pack16to32(mx,my), 4);
                    }
                }
            }
        }
		else
		{
            //for(list=0; list<Listcount; list++)
			{
                if(h->ref_count[0]>0)
				{
                    for(i=0; i<2; i++)
					{
						...
						{ //FIXME optimize
                            const int val= get_te0_golomb(&s->gb, h->ref_count[0]);
                            fill_rectangle(&h->ref_cache[0][ scan8[0] + 2*i ], 2, 4, 8, val, 1);
                        }
                    }
                }
            }
            //for(list=0; list<Listcount; list++)
			{
                for(i=0; i<2; i++)
				{
                    if(IS_DIR(mb_type, i, 0))
					{
                        pred_8x16_motion(h, i*4, h->ref_cache[0][ scan8[0] + 2*i ], &mx, &my);
                        mx += get_se_golomb(&s->gb);
                        my += get_se_golomb(&s->gb);
						
                        fill_rectangle(h->mv_cache[0][ scan8[0] + 2*i ], 2, 4, 8, pack16to32(mx,my), 4);
                    }
                }
            }
        }
...
static int decode_cabac_mb_type( H264Context *h ) 
{
    ...
    if( h->slice_type == I_TYPE )
	{
        ...
        if( s->mb_x > 0 && !IS_INTRA4x4( s->current_picture_ptr->mb_type[mb_xy-1] ) )
            ctx++;
        if( s->mb_y > 0 && !IS_INTRA4x4( s->current_picture_ptr->mb_type[mb_xy-s->mb_stride] ) )
            ctx++;
...
static int decode_cabac_mb_skip( H264Context *h)
{
    ...
    if( s->mb_x > 0 && !IS_SKIP( s->current_picture_ptr->mb_type[mba_xy] ) )
        ctx++;
    if( s->mb_y > 0 && !IS_SKIP( s->current_picture_ptr->mb_type[mbb_xy] ) )
        ctx++;
...
static int decode_cabac_mb_chroma_pre_mode( H264Context *h)
{
    ...
    if( s->mb_x > 0 &&
        ( IS_INTRA4x4( s->current_picture_ptr->mb_type[mba_xy] ) || IS_INTRA16x16( s->current_picture_ptr->mb_type[mba_xy] ) ) &&
        h->chroma_pred_mode_table[mba_xy] != 0 ) {
        ctx++;
    }
    if( s->mb_y > 0 &&
        ( IS_INTRA4x4( s->current_picture_ptr->mb_type[mbb_xy] ) || IS_INTRA16x16( s->current_picture_ptr->mb_type[mbb_xy] ) ) &&
...
static int decode_cabac_mb_cbp_luma( H264Context *h)
{
    ...
    for( i8x8 = 0; i8x8 < 4; i8x8++ )
	{
        ...
        if( mba_xy >= 0 )
		{
            ...
            if( IS_SKIP( s->current_picture_ptr->mb_type[mba_xy] ) || ((h->cbp_table[mba_xy] >> i8x8a)&0x01) == 0 )
                ctx++;
        }
        ...
        if( mbb_xy >= 0 )
		{
            ...
            if( IS_SKIP( s->current_picture_ptr->mb_type[mbb_xy] ) || ((h->cbp_table[mbb_xy] >> i8x8b)&0x01) == 0 )
                ctx += 2;
        }
...
static int decode_cabac_mb_cbp_chroma( H264Context *h)
{
    ...
    if( s->mb_x > 0 && !IS_SKIP( s->current_picture_ptr->mb_type[mb_xy-1] ) )
        cbp_a = (h->cbp_table[mb_xy-1]>>4)&0x03;
    else
        cbp_a = -1;
	
    if( s->mb_y > 0 && !IS_SKIP( s->current_picture_ptr->mb_type[mb_xy-s->mb_stride] ) )
...
static int decode_cabac_mb_dqp( H264Context *h) 
{
    ...
    if( mbn_xy >= 0 && h->last_qscale_diff != 0 && ( IS_INTRA16x16(s->current_picture_ptr->mb_type[mbn_xy] ) || (h->cbp_table[mbn_xy]&0x3f) ) )
        ctx++;
...
static int decode_cabac_mb_ref( H264Context *h, int n )
{
    int refa = h->ref_cache[0][scan8[n] - 1];
    int refb = h->ref_cache[0][scan8[n] - 8];
    int ref  = 0;
    int ctx  = 0;
...
static int decode_cabac_mb_mvd( H264Context *h, int n, int l )
{
    int amvd = abs( h->mvd_cache[scan8[n] - 1][l] ) +
		abs( h->mvd_cache[scan8[n] - 8][l] );
...
static int get_cabac_cbf_ctx( H264Context *h, int cat, int idx )
{
    ...
    if( cat == 0 )
	{
		...
		{
            mba_xy = mb_xy - 1;
            if( IS_INTRA16x16(s->current_picture_ptr->mb_type[mba_xy] ) )
				nza = h->cbp_table[mba_xy]&0x100;
        }
        if( s->mb_y > 0 )
		{
            mbb_xy = mb_xy - s->mb_stride;
            if( IS_INTRA16x16(s->current_picture_ptr->mb_type[mbb_xy] ) )
	...
	else if( cat == 1 || cat == 2 )
	{
        ...
        if( mba_xy >= 0 )
		{
            ...
            if( !IS_SKIP(s->current_picture_ptr->mb_type[mba_xy] ) &&
                !IS_INTRA_PCM(s->current_picture_ptr->mb_type[mba_xy] ) &&
        ...
        if( mbb_xy >= 0 )
		{
            ...
            if( !IS_SKIP(s->current_picture_ptr->mb_type[mbb_xy] ) &&
                !IS_INTRA_PCM(s->current_picture_ptr->mb_type[mbb_xy] ) &&
	...
	else if( cat == 4 )
	{
        ...
        if( mba_xy >= 0 &&
            !IS_SKIP(s->current_picture_ptr->mb_type[mba_xy] ) &&
            !IS_INTRA_PCM(s->current_picture_ptr->mb_type[mba_xy] ) &&
            (h->cbp_table[mba_xy]&0x30) == 0x20 )
            nza = h->non_zero_count_cache[scan8[16+idx] - 1];
		
        if( mbb_xy >= 0 &&
            !IS_SKIP(s->current_picture_ptr->mb_type[mbb_xy] ) &&
            !IS_INTRA_PCM(s->current_picture_ptr->mb_type[mbb_xy] ) &&
            (h->cbp_table[mbb_xy]&0x30) == 0x20 )
            nzb = h->non_zero_count_cache[scan8[16+idx] - 8];
    }
	
    if( ( mba_xy < 0 && IS_INTRA( s->current_picture_ptr->mb_type[mb_xy] ) ) ||
        ( mba_xy >= 0 && IS_INTRA_PCM(s->current_picture_ptr->mb_type[mba_xy] ) ) ||
		nza > 0 )
        ctx++;
	
    if( ( mbb_xy < 0 && IS_INTRA( s->current_picture_ptr->mb_type[mb_xy] ) ) ||
        ( mbb_xy >= 0 && IS_INTRA_PCM(s->current_picture_ptr->mb_type[mbb_xy] ) ) ||
		nzb > 0 )
        ctx += 2;
...
static int decode_mb_cabac(H264Context *h)
{
    ...
    if( h->slice_type != I_TYPE && h->slice_type != SI_TYPE )
	{
        ...
        if( decode_cabac_mb_skip( h ) ) 
		{
            ...
            pred_pskip_motion(h, &mx, &my);
            fill_rectangle(&h->ref_cache[0][scan8[0]], 4, 4, 8, 0, 1);
            fill_rectangle(  h->mvd_cache[scan8[0]], 4, 4, 8, pack16to32(0,0), 4);
            fill_rectangle(  h->mv_cache[0][scan8[0]], 4, 4, 8, pack16to32(mx,my), 4);
            write_back_motion(h, mb_type);
        ...
        //for( list = 0; list < Listcount; list++ ) 
		{
            if( h->ref_count[0] > 0 ) 
			{
                for( i = 0; i < 4; i++ ) 
				{
					...
					{
                        if( h->ref_count[0] > 1 )
                            ref[0][i] = decode_cabac_mb_ref( h, 4*i );
                        else
                            ref[0][i] = 0;
                    }
					else 
            ...
            for(i=0; i<4; i++)
			{
                if(IS_DIR(h->sub_mb_type[i], 0, 0) && !IS_DIRECT(h->sub_mb_type[i]))
				{
                    ...
                    for(j=0; j<sub_partition_count[i]; j++)
					{
                        ...
                        const int index= 4*i + block_width*j;
                        int16_t (* mv_cache)[2]= &h->mv_cache[0][ scan8[index] ];
                        int16_t (* mvd_cache)[2]= &h->mvd_cache[ scan8[index] ];
                        pred_motion(h, index, block_width, h->ref_cache[0][ scan8[index] ], &mpx, &mpy);
						
                        mx = mpx + decode_cabac_mb_mvd( h, index, 0 );
                        my = mpy + decode_cabac_mb_mvd( h, index, 1 );
...
//                            assert(IS_SUB_4X4(sub_mb_type));
				...
				{
                    uint32_t *p= (uint32_t *)&h->mv_cache[0][ scan8[4*i] ][0];
                    uint32_t *pd= (uint32_t *)&h->mvd_cache[ scan8[4*i] ][0];
                    p[0] = p[1] = p[8] = p[9] = 0;
        ...
        if(IS_16X16(mb_type))
		{
            //for(list=0; list<Listcount; list++)
			{
                if(IS_DIR(mb_type, 0, 0))
				{
                    if(h->ref_count[0] > 0 )
					{
                        const int ref = h->ref_count[0] > 1 ? decode_cabac_mb_ref( h, 0 ) : 0;
            ...
            //for(list=0; list<Listcount; list++)
			{
                if(IS_DIR(mb_type, 0, 0))
				{
                    pred_motion(h, 0, 4, h->ref_cache[0][ scan8[0] ], &mpx, &mpy);
					
                    mx = mpx + decode_cabac_mb_mvd( h, 0, 0 );
                    my = mpy + decode_cabac_mb_mvd( h, 0, 1 );
					
                    fill_rectangle(h->mvd_cache[ scan8[0] ], 4, 4, 8, pack16to32(mx-mpx,my-mpy), 4);
                    fill_rectangle(h->mv_cache[0][ scan8[0] ], 4, 4, 8, pack16to32(mx,my), 4);
                }
            }
        }
        else if(IS_16X8(mb_type))
		{
            //for(list=0; list<Listcount; list++)
			{
                if(h->ref_count[0]>0)
				{
                    for(i=0; i<2; i++)
					{
                        if(IS_DIR(mb_type, i, 0))
						{
                            const int ref= h->ref_count[0] > 1 ? decode_cabac_mb_ref( h,8*i ) : 0;
                            fill_rectangle(&h->ref_cache[0][ scan8[0] + 16*i ], 4, 2, 8, ref, 1);
                        }
                    }
                }
            }
            //for(list=0; list<Listcount; list++)
			{
                for(i=0; i<2; i++)
				{
                    if(IS_DIR(mb_type, i, 0))
					{
                        pred_16x8_motion(h, 8*i, h->ref_cache[0][scan8[0] + 16*i], &mpx, &mpy);
                        mx = mpx + decode_cabac_mb_mvd( h, 8*i, 0 );
                        my = mpy + decode_cabac_mb_mvd( h, 8*i, 1 );
						
                        fill_rectangle(h->mvd_cache[ scan8[0] + 16*i ], 4, 2, 8, pack16to32(mx-mpx,my-mpy), 4);
                        fill_rectangle(h->mv_cache[0][ scan8[0] + 16*i ], 4, 2, 8, pack16to32(mx,my), 4);
                    }
                }
            }
...
//            assert(IS_8X16(mb_type));
            //for(list=0; list<Listcount; list++)
			{
                if(h->ref_count[0]>0)
				{
                    for(i=0; i<2; i++)
					{
                        if(IS_DIR(mb_type, i, 0))
						{ //FIXME optimize
                            const int ref= h->ref_count[0] > 1 ? decode_cabac_mb_ref( h,4*i ) : 0;
                            fill_rectangle(&h->ref_cache[0][ scan8[0] + 2*i ], 2, 4, 8, ref, 1);
                        }
                    }
                }
            }
            //for(list=0; list<Listcount; list++)
			{
                for(i=0; i<2; i++)
				{
                    if(IS_DIR(mb_type, i, 0))
					{
                        pred_8x16_motion(h, i*4, h->ref_cache[0][ scan8[0] + 2*i ], &mpx, &mpy);
                        mx = mpx + decode_cabac_mb_mvd( h, 4*i, 0 );
                        my = mpy + decode_cabac_mb_mvd( h, 4*i, 1 );
						
                        fill_rectangle(h->mvd_cache[ scan8[0] + 2*i ], 2, 4, 8, pack16to32(mx-mpx,my-mpy), 4);
                        fill_rectangle(h->mv_cache[0][ scan8[0] + 2*i ], 2, 4, 8, pack16to32(mx,my), 4);
                    }
                }
            }
...
static void filter_mb( H264Context *h, int mb_x, int mb_y, uint8_t *img_y, uint8_t *img_cb, uint8_t *img_cr)
{
	...
	for( dir = 0; dir < 2; dir++ )
	{
		...
		for( edge = start; edge < 4; edge++ )
		{
			...
			if( IS_INTRA( s->current_picture_ptr->mb_type[mb_xy] ) || IS_INTRA( s->current_picture_ptr->mb_type[mbn_xy] ) )
			{
			...
			/* Filter edge */
			qp = ( s->qscale + s->current_picture_ptr->qscale_table[mbn_xy] + 1 ) >> 1;
			if( dir == 0 )
			{
				filter_mb_edgev( h, &img_y[4*edge], linesize, bS, qp );
				if( (edge&1) == 0 )
				{
					int chroma_qp = ( h->chroma_qp + get_chroma_qp( h, s->current_picture_ptr->qscale_table[mbn_xy] ) + 1 ) >> 1;
...
static  void hl_decode_mb(H264Context *h)
{
    ...
    dest_y  = s->current_picture_ptr->data[0] + (mb_y * 16* Ylinesize ) + mb_x * 16;
    dest_cb = s->current_picture_ptr->data[1] + (mb_y * 8 * UVlinesize) + mb_x * 8;
    dest_cr = s->current_picture_ptr->data[2] + (mb_y * 8 * UVlinesize) + mb_x * 8;
	
    if (h->mb_field_decoding_flag)
	{
        linesize = Ylinesize * 2;
        uvlinesize = UVlinesize * 2;
        if(mb_y&1)
...
static int decode_seq_parameter_set(H264Context *h)
{
	...
	get_bits1(&s->gb);
	mytemp= get_bits1(&s->gb);
	if(mytemp)//if(sps->crop)
	{
...
int decode_frame(AVCodecContext *avctx, void *data, int *data_size,uint8_t *buf, int buf_size)
{
	...
	*data_size = 0;
	...
	if(!s->current_picture_ptr)return -1;	
