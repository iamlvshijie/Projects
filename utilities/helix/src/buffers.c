...
MP3DecInfo *AllocateBuffers(void)
{
  ...
  ClearBuffer((void*)&mp3DecInfo, sizeof(MP3DecInfo));
   ...
   // return mp3DecInfo_pointer;
	...
	mp3DecInfo_pointer = (MP3DecInfo *)mymalloc(ALLOCRAM,sizeof(MP3DecInfo));
	if (!mp3DecInfo_pointer)
		return 0;
	ClearBuffer(mp3DecInfo_pointer, sizeof(MP3DecInfo));
	
	fh =  (FrameHeader *)     mymalloc(ALLOCRAM,sizeof(FrameHeader));
	si =  (SideInfo *)        mymalloc(ALLOCRAM,sizeof(SideInfo));
	sfi = (ScaleFactorInfo *) mymalloc(ALLOCRAM,sizeof(ScaleFactorInfo));
	hi =  (HuffmanInfo *)     mymalloc(ALLOCRAM,sizeof(HuffmanInfo));
	di =  (DequantInfo *)     mymalloc(ALLOCRAM,sizeof(DequantInfo));
	mi =  (IMDCTInfo *)       mymalloc(ALLOCRAM,sizeof(IMDCTInfo));
	sbi = (SubbandInfo *)     mymalloc(ALLOCRAM,sizeof(SubbandInfo));
...
void FreeBuffers(MP3DecInfo *mp3DecInfo)
{
	...
	myfree(ALLOCRAM,mp3DecInfo->FrameHeaderPS);
	myfree(ALLOCRAM,mp3DecInfo->SideInfoPS);
	myfree(ALLOCRAM,mp3DecInfo->ScaleFactorInfoPS);
	myfree(ALLOCRAM,mp3DecInfo->HuffmanInfoPS);
	myfree(ALLOCRAM,mp3DecInfo->DequantInfoPS);
	myfree(ALLOCRAM,mp3DecInfo->IMDCTInfoPS);
	myfree(ALLOCRAM,mp3DecInfo->SubbandInfoPS);

	myfree(ALLOCRAM,mp3DecInfo);
#endif
}
