 ...
 */
  ...
  if ( NES->FrameCnt == 0 &&
       NES->PPU_ScanTable[ NES->PPU_Scanline ] == SCAN_ON_SCREEN )
  {
    if(NES->PPU_Scanline%2)InfoNES_DrawLine();
  }
...
//       }
    ...
    case SCAN_VBLANK_START:             //FrameSkip
      NES->FrameCnt = ( NES->FrameCnt >= 2 ) ? 0 : NES->FrameCnt + 1;
      // Set a V-Blank flag
      NES->PPU_R2 = R2_IN_VBLANK;
      // Reset latch flag
      NES->PPU_Latch_Flag = 0;
      // pAPU Sound function in V-Sync
      if ( !NES->APU_Mute )InfoNES_pAPUVsync();
 ...
 */
  ...
  pPoint = NES->Linebuf;//&WorkFrame[ PPU_Scanline * NES_DISP_WIDTH ];
  ...
  for(i=0;i<160;i++)LCD_WR_DATA(NES->Linebuf[(i<<8)/160]);
}
